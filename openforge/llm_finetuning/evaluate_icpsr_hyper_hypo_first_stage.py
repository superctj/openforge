...
def get_predictions(
    ...
    input_df["first_stage_prior_prediction"] = preds
    input_df["first_stage_prior_confidence_score"] = confdc_scores

    labels = input_df["first_stage_label"].tolist()
...
if __name__ == "__main__":
    ...
    valid_df["first_stage_label"] = valid_df.apply(
        lambda x: 0 if x["relation_variable_label"] == 0 else 1, axis=1
    )
    ...
    test_df["first_stage_label"] = test_df.apply(
        lambda x: 0 if x["relation_variable_label"] == 0 else 1, axis=1
    )
