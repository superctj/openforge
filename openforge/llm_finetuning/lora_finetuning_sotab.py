...
if __name__ == "__main__":
    ...
    train_df, valid_df, test_df = load_openforge_sotab_benchmark(
        os.path.join(config.get("exp", "data_dir"), "artifact")
    )

    train_df = prepare_sotab_for_sequence_classification(
        train_df, root_dir=config.get("exp", "data_dir")
    )
    valid_df = prepare_sotab_for_sequence_classification(
        valid_df, root_dir=config.get("exp", "data_dir")
    )
    test_df = prepare_sotab_for_sequence_classification(
        test_df, root_dir=config.get("exp", "data_dir")
    )
