...
def get_llm_prediction(
    ...
    sample_preds = np.array(
        [parse_llm_response(sample) for sample in responses]
    )
    pred = np.argmax(np.bincount(sample_preds))
    pred_confdc_score = np.mean(sample_preds == pred)

    if logger:
        for sample in responses:
            logger.info(f"Response: {sample}")

    return pred, pred_confdc_score
