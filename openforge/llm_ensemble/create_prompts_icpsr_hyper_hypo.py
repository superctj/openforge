...
from openforge.utils.llm_common import (
    craft_icpsr_hyper_hypo_user_prompt,
    load_icpsr_hyper_hypo_dataset,
)
from openforge.utils.util import parse_config


def sample_few_shot_examples(
    df: pd.DataFrame, n: int = 15, balanced: bool = True, random_seed: int = 42
) -> pd.DataFrame:
    if n == 0:
        return pd.DataFrame()
    ...
    sample_df = pd.concat(
        [
            df[df["relation_variable_label"] == 2].sample(
...
def create_prompts_per_split(
    ...
    for i, row in input_df.iterrows():
        prompt = craft_icpsr_hyper_hypo_user_prompt(row, few_shot_df)
