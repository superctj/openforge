...
def synthesize_pos_and_neg_pairs(data_dir: str, output_filepath: str):
    schemaorg_metadata_filepath = os.path.join(
        data_dir, "cta_schemaorg/sotab_v2_cta_test_set.csv"
    )
    dbpedia_metadata_filepath = os.path.join(
        data_dir, "cta_dbpedia/sotab_cta_test_dbpedia.csv"
    )
    ...
    NegEntry = make_dataclass(
        "NegEntry",
        [
            ("schemaorg_table_name", str),
            ("schemaorg_column_index", int),
            ("schemaorg_label", str),
            ("dbpedia_table_name", str),
            ("dbpedia_column_index", int),
            ("dbpedia_label", str),
        ],
    )
    pos_data = []
    ...
    for row in dbpedia_df.itertuples():
        ...
        while True:
            ...
            if schemaorg_col != unique_id:
                schemaorg_table_name, schemaorg_column_index = (
                    schemaorg_col.split(DELIMITER)
                )
                schemaorg_label = schemaorg_col_label_mapping[schemaorg_col]

                neg_data.append(
                    NegEntry(
                        schemaorg_table_name,
                        int(schemaorg_column_index),
                        schemaorg_label,
                        row.table_name,
                        row.column_index,
                        row.label,
...
def synthesize_evaluation_dataset(
    pos_df: pd.DataFrame,
    neg_df: pd.DataFrame,
    ...
    for row in pos_df.itertuples():
        ...
        table = pd.read_json(
            os.path.join(dbpedia_table_dir, table_name),
            compression="gzip",
            lines=True,
        )
        ...
        if table.shape[0] < args.value_sample_size * 2:
            ...
            sample_values = (
                table.iloc[:, col_idx]
                .sample(
                    n=args.value_sample_size * 2, random_state=args.random_seed
                )
                .tolist()
            )
        ...
        schemaorg_name_signature = set(
            qgram_transformer.transform(row.schemaorg_label)
        )
        schemaorg_fasttext_signature = fasttext_transformer.transform(
            schemaorg_values
        )
        ...
        name_sim = jaccard_index(
            schemaorg_name_signature, dbpedia_name_signature
        )
        value_sim = cosine_similarity(
            schemaorg_fasttext_signature, dbpedia_fasttext_signature
        )
    ...
    for row in neg_df.itertuples():
        ...
        schemaorg_name_signature = set(
            qgram_transformer.transform(schemaorg_label)
        )
        dbpedia_name_signature = set(qgram_transformer.transform(dbpedia_label))

        schemaorg_table = pd.read_json(
            os.path.join(schemaorg_table_dir, schemaorg_table_name),
            compression="gzip",
            lines=True,
        )
        ...
        if schemaorg_table.shape[0] < args.value_sample_size:
            schemaorg_values = schemaorg_table.iloc[
                :, schemaorg_col_idx
            ].tolist()
        else:
            schemaorg_values = (
                schemaorg_table.iloc[:, schemaorg_col_idx]
                .sample(n=args.value_sample_size, random_state=args.random_seed)
                .tolist()
            )
        ...
        dbpedia_table = pd.read_json(
            os.path.join(dbpedia_table_dir, dbpedia_table_name),
            compression="gzip",
            lines=True,
        )
        ...
        if dbpedia_table.shape[0] < args.value_sample_size:
            ...
            dbpedia_values = (
                dbpedia_table.iloc[:, dbpedia_col_idx]
                .sample(n=args.value_sample_size, random_state=args.random_seed)
                .tolist()
            )
    ...
    with open(
        os.path.join(
            args.output_dir, "sotab_v2_cta_test_synthesized_microbenchmark.pkl"
        ),
        "wb",
    ) as f:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default="Test",
        help="Split of raw data for synthesizing the microbenchmark.",
    )
    ...
    parser.add_argument(
        "--log_dir",
        type=str,
        default="/home/congtj/openforge/logs/sotab_synthesized_data",
    ...
    label_mapping_filepath = os.path.join(
        args.output_dir, "sotab_v2_cta_test_synthesized"
    )
    pos_df, neg_df = synthesize_pos_and_neg_pairs(
        args.data_dir, label_mapping_filepath
    )

    schemaorg_table_dir = os.path.join(
        args.data_dir, f"cta_schemaorg/{args.split}"
    )
    dbpedia_table_dir = os.path.join(args.data_dir, f"cta_dbpedia/{args.split}")
    synthesize_evaluation_dataset(
        pos_df, neg_df, schemaorg_table_dir, dbpedia_table_dir, logger, args
    )
