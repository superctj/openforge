        ...
        for j, reference_concept in enumerate(node_i.texts):
            ...
            for k, merged_concept in enumerate(node_i.texts[j+1:]):
                ...
                if len(merged_concept_fasttext_signature) == 0:
                    logger.info(
                        f"Cannot compute fasttext signature for merged concept: {merged_concept}."
                    )
                    continue
            ...
            for subsequent_node in arts_nodes[i+1:]:
                for unmerged_concept in subsequent_node.texts:
                    logger.info(
                        f"Concept pair (with global concept id {reference_concept_id} and {unmerged_concept_id}): {reference_concept} and {unmerged_concept}"
                    )
                    ...
                    if len(unmerged_concept_fasttext_signature) == 0:
                        logger.info(
                            f"Cannot compute value signature for unmerged concept: {unmerged_concept}."
                        )
                        continue
