...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--mrf_boost_data",
        type=str,
        default="/home/congtj/openforge/exps/arts_top-10-concepts/arts_test_mrf_data_with_mrf_predictions.csv",
        help="Path to subset predictions for boosting test dataset inference."
    )

    parser.add_argument(
        "--mrf_test_data",
        type=str,
        default="/home/congtj/openforge/exps/arts_top-15-concepts/arts_test_mrf_data_with_confidence_scores.csv", # "/home/congtj/openforge/exps/arts_top-100-concepts/sotab_v2_test_mrf_data_with_confidence_scores.csv"
        help="Path to synthesized MRF data for testing."
    )

    parser.add_argument(
        "--log_dir",
        type=str,
        default="/home/congtj/openforge/logs/arts_mrf_synthesized_data_top-15-concepts/mrf_inference", # "/home/congtj/openforge/logs/sotab_synthesized_data/mrf_inference"
    ...
    mrf_boost_df = pd.read_csv(args.mrf_boost_data)
    mrf_test_df = pd.read_csv(args.mrf_test_data)
    ...
    for row in mrf_test_df.itertuples():
    ...
    for row in mrf_test_df.itertuples():
        ...
        if row.relation_variable_label != pred:
            logger.info(f"Posterior prediction is incorrect.")
    
    mrf_test_df["mrf_prediction"] = y_pred
    output_filepath = args.mrf_test_data.split("/")[:-1]
    output_filepath = os.path.join(
        "/".join(output_filepath),
        "arts_test_mrf_data_with_mrf_predictions.csv"
    )
    mrf_test_df.to_csv(output_filepath, index=False)

    logger.info("-"*80)
    logger.info(f"Number of test instances: {len(mrf_test_df)}")
