...
def synthesize_evaluation_dataset(pos_df: pd.DataFrame, neg_df: pd.DataFrame, schemaorg_table_dir: str, dbpedia_table_dir: str, logger, args: argparse.Namespace):
    ...
    for row in pos_df.itertuples():
        ...
        dbpedia_name_signature = set(
            qgram_transformer.transform(dbpedia_label)
        )
        ...
        name_sim = jaccard_index(
            schemaorg_name_signature,
            dbpedia_name_signature
        )
        value_sim = cosine_similarity(
            schemaorg_fasttext_signature,
            dbpedia_fasttext_signature
        )
