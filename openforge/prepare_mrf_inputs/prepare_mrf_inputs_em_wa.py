        ...
        for _, row in group.iterrows():
            if row["prediction"] == 1:
                ...
                for pair in combinations(mrf_r_id, 2):
                    ...
                    extra_inputs.append(
                        f"Entity 1: {r_id_entity_map[l_rid]}; Entity 2: {r_id_entity_map[r_rid]}.{embedding_model.tokenizer.eos_token}"  # noqa: E501
                    )

                with torch.no_grad():
                    extra_embeddings = embedding_model.encode(
                        extra_inputs,
                        batch_size=batch_size,
                        prompt=ENTITY_MATCHING_INSTRUCTION,
                        device=device,
                ...
                prior_pred, prior_confdc_scores = get_ridge_prior_predictions(
